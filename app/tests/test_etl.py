import json
import pandas as pd
from app.etl import load_events, filter_consented, aggregate_by_campaign, build_activation_payload
...
def test_aggregate_and_payload(tmp_path):
    ...
    p.write_text(
        '\n'.join([
            json.dumps({"campaign":"X", "consent": True, "click":1, "conversion":0}),
            json.dumps({"campaign":"X", "consent": True, "click":1, "conversion":1}),
            json.dumps({"campaign":"Y", "consent": True, "click":0, "conversion":1}),
        ]),
        encoding="utf-8"
    )
    df = load_events(str(p))
