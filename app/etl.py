...
def filter_consented(df):
  # Bug: consent sometimes arrives as strings "true"/"false"; we should normalize
  # Also: function should drop records with missing consent
...
def build_activation_payload(df):
    ...
    for _, r in df.iterrows():
        payload.append({
            "campaign": r["campaign"],
            "metrics": {"clicks": int(r["clicks"]), "conversions": int(r["conversions"])}
        })
    return payload
